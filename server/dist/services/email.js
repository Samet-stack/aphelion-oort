...
export const generateEmailVerification = async (userId, email) => {
    ...
    await run(`INSERT INTO email_verifications (id, user_id, email, token, expires_at)
     VALUES (?, ?, ?, ?, ?)`, [uuidv4(), userId, email.toLowerCase(), token, expiresAt.toISOString()]);
    return token;
};
// Vérifier un token
export const verifyEmailToken = async (token) => {
    const verification = await get(`SELECT * FROM email_verifications 
     WHERE token = ? AND used = false AND expires_at > NOW()`, [token]);
    if (!verification) {
        return { valid: false, message: 'Token invalide ou expiré' };
    }
    // Marquer comme utilisé
    await run('UPDATE email_verifications SET used = true, verified_at = NOW() WHERE id = ?', [verification.id]);
    // Activer l'utilisateur
    await run('UPDATE users SET email_verified = true, updated_at = NOW() WHERE id = ?', [verification.userId]);
