...
router.get('/', async (req, res) => {
    try {
        ...
        const plans = await query(`SELECT id, user_id, site_name, address, created_at, updated_at
       FROM plans WHERE user_id = ?
       ORDER BY created_at DESC`, [userId]);
        for (const plan of plans) {
            const countResult = await get('SELECT COUNT(*) as count FROM plan_points WHERE plan_id = ?', [plan.id]);
...
router.get('/:id', async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const plan = await get('SELECT * FROM plans WHERE id = ? AND user_id = ?', [id, userId]);
        if (!plan) {
        ...
        const points = await query('SELECT * FROM plan_points WHERE plan_id = ? ORDER BY point_number ASC', [id]);
        plan.points = points;
...
router.post('/', async (req, res) => {
    try {
        ...
        const id = uuidv4();
        await run(`INSERT INTO plans (id, user_id, site_name, address, image_data_url)
       VALUES (?, ?, ?, ?, ?)`, [id, userId, siteName, address || null, imageDataUrl]);
...
router.delete('/:id', async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const plan = await get('SELECT id FROM plans WHERE id = ? AND user_id = ?', [id, userId]);
        if (!plan) {
            return res.status(404).json({
                success: false,
                message: 'Plan non trouvé.'
...
router.post('/:id/points', async (req, res) => {
    try {
        ...
        const plan = await get('SELECT id FROM plans WHERE id = ? AND user_id = ?', [planId, userId]);
        if (!plan) {
            return res.status(404).json({
                success: false,
                message: 'Plan non trouvé.'
        ...
        const maxResult = await get('SELECT COALESCE(MAX(point_number), 0) as max_num FROM plan_points WHERE plan_id = ?', [planId]);
        const pointNumber = Number(maxResult.maxNum || 0) + 1;
        const pointId = uuidv4();
        await run(`INSERT INTO plan_points (id, plan_id, user_id, position_x, position_y, title, description,
        category, photo_data_url, date_label, room, status, point_number)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [pointId, planId, userId, positionX, positionY, title, description || null,
...
router.put('/:id/points/:pointId', async (req, res) => {
    try {
        ...
        const point = await get('SELECT id FROM plan_points WHERE id = ? AND plan_id = ? AND user_id = ?', [pointId, planId, userId]);
        if (!point) {
            return res.status(404).json({
                success: false,
        ...
        await run(`UPDATE plan_points SET
       ...
       WHERE id = ?`, [title || null, description, category || null, photoDataUrl || null,
