...
router.get('/excel', async (req, res) => {
    try {
       ...
       ORDER BY r.created_at DESC`, params);
        ...
        const extraWorks = await query(`SELECT 
        ew.*,
       ...
       ORDER BY ew.created_at DESC`, params);
        // Statistiques
       ...
       FROM reports
       WHERE user_id = ? ${dateFilter.replace(/r\.created_at/g, 'created_at')}`, params);
        res.json({
            success: true,
            data: {
                reports,
...
router.get('/report/:id', async (req, res) => {
    try {
        ...
        const report = await query(`SELECT r.* FROM reports r
       LEFT JOIN shares s ON r.id = s.report_id AND (s.shared_with_id = ? OR s.shared_with_email = ?) AND s.status = 'active'
       WHERE (r.user_id = ? OR s.id IS NOT NULL) AND r.id = ?`, [userId, req.user.email, userId, id]);
