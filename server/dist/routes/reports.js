...
router.get('/', async (req, res) => {
    try {
       ...
       LIMIT ? OFFSET ?`, [userId, parseInt(limit), parseInt(offset)]);
        // Pour chaque rapport, récupérer les travaux supplémentaires
        for (const report of reports) {
            const extraWorks = await query('SELECT * FROM extra_works WHERE report_id = ?', [report.id]);
...
router.get('/:id', async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const report = await get('SELECT * FROM reports WHERE id = ? AND user_id = ?', [id, userId]);
        if (!report) {
        ...
        const extraWorks = await query('SELECT * FROM extra_works WHERE report_id = ?', [id]);
        report.extraWorks = extraWorks;
        res.json({
            success: true,
            data: { report }
...
router.post('/', async (req, res) => {
    try {
        const userId = req.user.id;
        const { reportId, dateLabel, address, coordinates, accuracy, locationSource, description, imageDataUrl, siteName, operatorName, clientName, priority, category, integrityHash, clientSignature, extraWorks = [] } = req.body;
        const id = uuidv4();
        await run(`INSERT INTO reports (
        ...
        for (const work of extraWorks) {
            await run(`INSERT INTO extra_works (id, report_id, user_id, description, estimated_cost, urgency, category)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [uuidv4(), id, userId, work.description, work.estimatedCost, work.urgency, work.category]);
        }
...
router.get('/stats/summary', async (req, res) => {
    try {
        const userId = req.user.id;
        const totalReports = await get('SELECT COUNT(*) as count FROM reports WHERE user_id = ?', [userId]);
        const totalExtraWorks = await get('SELECT COUNT(*) as count FROM extra_works WHERE user_id = ?', [userId]);
        const totalValue = await get('SELECT SUM(estimated_cost) as total FROM extra_works WHERE user_id = ?', [userId]);
        const byCategory = await query(`SELECT category, COUNT(*) as count 
       FROM reports 
