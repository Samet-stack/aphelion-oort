...
const getPool = () => {
    ...
    if (!databaseUrl) {
        throw new Error('SUPABASE_DB_URL (ou DATABASE_URL) est requis pour se connecter a PostgreSQL.');
    }
    if (!pool) {
...
const toCamelObject = (obj) => Object.fromEntries(Object.entries(obj).map(([k, v]) => [toCamel(k), v]));
