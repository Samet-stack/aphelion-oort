...
router.post('/', async (req, res) => {
  try {
    ...
    await run(
      `INSERT INTO shares (id, report_id, owner_id, shared_with_email, shared_with_id, permission, message, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        shareId,
        reportId,
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const share = await get(
      'SELECT id FROM shares WHERE id = ? AND owner_id = ?',
      [id, ownerId]
    );
...
router.put('/:id/accept', async (req, res) => {
  try {
    ...
    const share = await get(
      "SELECT * FROM shares WHERE id = ? AND shared_with_email = ? AND status = 'pending'",
      [id, userEmail]
    );
    ...
    await run(
      "UPDATE shares SET shared_with_id = ?, status = 'active', accepted_at = NOW() WHERE id = ?",
      [userId, id]
    );
